...
byCat
#byCat%>%   write.csv("../data/output/PercentTopicsByCategory.csv")
```
...
```{r}
GetFlagValue <- function(comm)
{
  uids <- commAssignments %>% filter(Community_ID ==comm) %>% pull(UID)
  crids <- allComplaints %>% filter(UID %in% uids) %>% pull(CRID)
  val <- flagged %>% filter(CRID %in% crids) %>% pull(Val)
  return(sum(val)/length(val))
}
commsAltered <- comms %>% group_by(Community_ID) %>% mutate(flagVal = GetFlagValue(Community_ID))
```
